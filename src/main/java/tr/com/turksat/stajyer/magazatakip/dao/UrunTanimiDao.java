...
public class UrunTanimiDao {
    ...
    public UrunTanimi getUrunTanimi(Integer urunTanimiId) {
        Connection con = null;
        PreparedStatement ps = null;
        UrunTanimi findUrunTanimi = null;
        try {
            con = Database.getInstance().getConnection();
            ps = con.prepareStatement(
                    "select * from urun_tanimi");
            ...
            while (rs.next()) // found
            {
                findUrunTanimi = new UrunTanimi();
                findUrunTanimi.setId(rs.getInt(1));
                findUrunTanimi.setUrunCesitId(rs.getInt(2));
                findUrunTanimi.setUrunRenk(rs.getString(3));
                findUrunTanimi.setUrunMarka(rs.getString(4));
                findUrunTanimi.setUrunModel(rs.getString(5));
                findUrunTanimi.setUretimTarihi(rs.getDate(6));
                findUrunTanimi.setSistemeEklemeTarihi(rs.getDate(7));
                findUrunTanimi.setUrunGarantiYil(rs.getInt(8));
            }
        } catch (Exception e) {
            System.out.println("Hatalı islem.");
        } finally {
